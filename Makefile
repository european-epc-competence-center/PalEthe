...
start_miner : geth/chaindata
	@ps -A | grep geth || nohup geth/geth --rpc --rpccorsdomain "*" --maxpeers 0 --nodiscover --datadir geth/chaindata --password $(PASSWDFILE) --unlock 0 --mine --minerthreads 1 --rpcapi eth,web3,personal,miner,net,txpool &
	@sleep 3 && ps -A | grep geth || echo Failed to start geth, see nohup.out
